    ...
    public void start() throws ClassNotFoundException, UnexpectedResponseTypeException, IOException {
        ...
        while(System.currentTimeMillis() < stopTime) {
            try {
                ...
                logger.error("Consumer: IOException, backing of for 1 sec", e);
                try {
                    Thread.sleep(1000);
        ...
        if (msg.text.length() > STOP_STRING.length()) {
            ...
            if(hexDigest.equals(bytesToHex(md.digest()))) {
                logger.trace(String.format(
                        "Transmission succeeded for client %s and queue %s: %d messages, client state: %s",
                        name, queueName, msgCount, hexDigest));
            } else {
                logger.error(String.format("Transmission failed for client %s and queue %s. %d messages consumed."+
